...
更新日期：2026-09-20。起点：`1b60ae8f916c48e25ae2dc2e1354ec586ca7133a`。
...
当前实施状态以第 2 节与第 15 节为准；下表保留建立本计划时的 R0–R6 缺口，不能作为
最新未完成项清单。主要有限范围实施已收口；S2 G2/G3、backend 拆分和 E1 保留触发条件，
第 15 节批次原 canary exit 1；第 16 节已在同一 base `4d85ed2` 补齐 64/64 绿色复核。
严格空闲窗口及更早独立批次的历史验收债务未据此撤销，详见
[架构收尾与证据计划](ARCHITECTURE_CLOSEOUT_2026_09.md)。

| 原阶段 | 建立计划时状态（历史） | 当时尚未关闭的目标 |
| --- | --- | --- |
...
| 6 | P1 / S3 | Flight 分批桥接与协议资源边界 | 查询执行至输出端有界；慢消费者背压、断连取消；schema 请求避免重复完整执行；不为嵌入式点查增加固定锁成本 | 已实现并功能验证（流式执行、有界通道、schema 缓存、Rust/Python 测试）；后续批次的增量 full 曾 exit 0（11.6、11.7、15.6），不追溯替代 S3 原批次验收；canary 的 host-load 干扰按 V1 记录（15.5） |
...
- `do_get` 在 `spawn_blocking` 里运行生产者，经容量 2 的 `tokio::sync::mpsc`
  通道喂给 gRPC 流：受支持流式形状的交付缓存受行组与通道容量约束，慢消费者形成背压，
  断连 drop 接收端后生产者在下个批次边界停止。非流式形状回落物化结果，并按
  65536 行切片交付，避免单个 Flight 消息过大；切片不消除完整结果物化的内存占用，
  不能将此处描述为全查询/进程 RSS 上限。
- `get_flight_info` / `get_schema` 对受支持流式形状避免完整执行：从首个行组批次
...
- **V1**（空闲窗口绿色门禁）：2026-09-19/20 的多个批次已在 full 完整模式多次 exit 0
  （见 11.6、11.7、15.6；14.3 的 M1 两轮 full 原始 exit 1），canary 仍受 host load 干扰；本轮在最终修订上继续复核，原始退出
...
## 16. 验收证据收尾与 V1 原基线复核（2026-09-20）

按先计划后代码的顺序执行[架构收尾计划](ARCHITECTURE_CLOSEOUT_2026_09.md)。本批只修改
计时区外的门禁证据工具、对应 Python 测试和文档；没有修改 Rust/Python 运行时与实际
benchmark 工作负载。新增持久化源码清单、patch、Cargo.lock、wheel 摘要及原始退出状态。

报告 `local-perf-results/closeout-acceptance-20260920-134803/`：release、pytest
1811、cargo 596+6、Flight 600+6 均通过且有原始日志；公开 benchmark 103/103 表格、
向量 6/6、量化 6/6 全部胜出。本轮 base `41f3400` 的 canary 五样本终判 64/64、full
三样本 109+10+8+4+4 全过，均 exit 0；V1 保持第 15 节原 base `4d85ed2`，canary
五样本终判 64/64、exit 0。所有初判失败与原始样本保留。

公开旧基线比较仍为 **exit 1（5 项）**，详细指标与 full 对照见收尾计划第 5 节；
旧公开基线 macOS 26.6.2、本次 27.0，同机 full 对应指标全过，不将旧基线差异归因于
本轮工具改动。V1 本次首尾 load 7.67/9.39，只关闭 remaining 批次“原基线 canary 尚无
绿色复核”的缺口，不宣称严格空闲窗口、整轮架构验收或更早失败批次全部闭环。
