//! Shared vector distance operations for ANN / TopK queries.
//!
//! Supports:
...
impl DistanceComputer {
    ...
    pub fn compute(&self, a: &[f32]) -> f32 {
        match self.metric {
            ...
            DistanceMetric::CosineDistance => (1.0
                - cosine_similarity_fused(a, &self.query, self.query_norm_recip))
            .clamp(0.0, 2.0),
        }
    }
    ...
    pub fn finalize_topk_distance(&self, score: f32) -> f32 {
        match self.metric {
            ...
            DistanceMetric::CosineSimilarity => -score,
            _ => score,
        }
    }
    ...
    pub fn compute_cosine_topk_ordering_from_dot(&self, dot: f32, row_norm_recip: f32) -> f32 {
        ...
        match self.metric {
            ...
            DistanceMetric::CosineDistance => (1.0 - sim).clamp(0.0, 2.0),
...
impl DistanceMetric {
    pub fn from_str(s: &str) -> Option<Self> {
        match s.to_uppercase().as_str() {
            ...
            "COSINE" | "COSINE_DISTANCE" | "ARRAY_COSINE_DISTANCE" => Some(Self::CosineDistance),
            "COSINE_SIMILARITY" | "ARRAY_COSINE_SIMILARITY" => Some(Self::CosineSimilarity),
            _ => None,
        }
    }
        ...
        match self {
            ...
            Self::CosineDistance => {
                ...
                if nb_sq == 0.0 {
                    ...
                    (1.0 - cosine_similarity_fused(a, b, 1.0 / nb_sq.sqrt())).clamp(0.0, 2.0)
                }
            }
        }
    }
...
pub fn topk_heap_direct(
    ...
    let mut result: Vec<(usize, f32)> = heap
        ...
        .collect();
    if metric == DistanceMetric::CosineSimilarity {
...
fn topk_heap_on_floats_impl(
    ...
    if computer.metric == DistanceMetric::CosineSimilarity {
        let per_chunk: Vec<Vec<TopKEntry>> = (0..t)
            .into_par_iter()
            .map(|tid| {
                let start = tid * chunk_size;
                ...
                for i in start..end {
                    ...
                    insert_topk_score(
                        &mut entries,
                        k_capped,
                        computer.compute_topk_ordering(vec),
                        i,
                    );
                }
                entries
...
fn topk_sequential_on_floats_impl(
    ...
    if computer.metric == DistanceMetric::CosineSimilarity {
        for i in 0..n_rows {
            ...
            let vec = unsafe { floats.get_unchecked(off..off + dim) };
            insert_topk_score(&mut entries, k, computer.compute_topk_ordering(vec), i);
        }
        return topk_entries_to_result(entries, computer);
    }
    for i in 0..n_rows {
        ...
        .map(|tid| {
            ...
            for i in start..end {
                    ...
                    .enumerate()
                {
                    ...
                    if dot_metric || metric == DistanceMetric::CosineSimilarity {
...
fn compute_f16_row_topk_ordering(row: &[u8], c: &DistanceComputer) -> f32 {
    ...
    if std::arch::is_aarch64_feature_detected!("fp16") {
        return match c.metric {
            ...
            DistanceMetric::CosineDistance => {
                (1.0 - unsafe { cosine_f16_neon(row, q, qnr) }).clamp(0.0, 2.0)
            }
    ...
    return match c.metric {
        ...
        DistanceMetric::CosineDistance => (1.0 - cosine_f16_scalar(row, q, qnr)).clamp(0.0, 2.0),
    ...
    #[cfg(not(target_arch = "aarch64"))]
    {
        ...
        if is_x86_feature_detected!("f16c") && is_x86_feature_detected!("avx2") {
            return match c.metric {
                ...
                DistanceMetric::CosineDistance => {
                    (1.0 - unsafe { cosine_f16_avx(row, q, qnr) }).clamp(0.0, 2.0)
                }
        ...
        match c.metric {
            ...
            DistanceMetric::CosineSimilarity => -cosine_f16_scalar(row, q, qnr),
            DistanceMetric::CosineDistance => {
                (1.0 - cosine_f16_scalar(row, q, qnr)).clamp(0.0, 2.0)
            }
            DistanceMetric::L1 => l1_f16_scalar(row, q),
            DistanceMetric::LInf => linf_f16_scalar(row, q),
        }
...
mod tests {
    ...
    #[test]
    fn test_cosine_metric_alias_and_similarity_topk_ordering() {
        assert_eq!(
        ...
        assert_eq!(
            DistanceMetric::from_str("cosine_similarity"),
            Some(DistanceMetric::CosineSimilarity)
        );

        let computer = DistanceComputer::new(DistanceMetric::CosineSimilarity, vec![1.0, 0.0]);
        assert!(
            computer.compute_topk_ordering(&[1.0, 0.0])
                < computer.compute_topk_ordering(&[-1.0, 0.0])
        );

        let rows = [1.0, 1.0, 0.9, 0.0];
        let top = topk_heap_on_floats(&rows, 2, 2, &computer, 1);
        assert_eq!(top[0].0, 1, "cosine TopK must normalize row magnitude");
        assert!(top[0].1 > 0.999, "normalized cosine score was {}", top[0].1);
    }
